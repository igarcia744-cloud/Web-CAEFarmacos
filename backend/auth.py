...
# Cargar variables desde el archivo seguridad.env
...
async def get_current_user(token: str):
    ...
    async with async_session() as db:
        result = await db.execute(select(Usuario).filter(Usuario.id == int(user_id)))
        usuario = result.scalars().first()
...
async def login(data: LoginData):
    async with async_session() as db:
        usuario = await autenticar_usuario(data.email, data.password, db)
